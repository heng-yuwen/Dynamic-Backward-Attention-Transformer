...
class DPGLTransformer(nn.Module, EncoderMixin):
    ...
    def __init__(self,
        ...
        self.enlarge_4x = EnlargeChannel(in_chans=self._out_channels[1], out_chans=self._out_channels[4], scale_rate=32//4)
        self.enlarge_8x = EnlargeChannel(in_chans=self._out_channels[2], out_chans=self._out_channels[4],
                                         scale_rate=32//8)
        self.enlarge_16x = EnlargeChannel(in_chans=self._out_channels[3], out_chans=self._out_channels[4],
                                         scale_rate=32//16)
    ...
    def att_forward(self, input):
        ...
        resized_patch_features.append(self.enlarge_4x(global_features[1]))
        resized_patch_features.append(self.enlarge_8x(global_features[2]))
        resized_patch_features.append(self.enlarge_16x(global_features[3]))
        resized_patch_features.append(global_features[4])
        ...
        for idx, patch_feature in enumerate(resized_patch_features):
            resized_patch_features[idx] = torch.mul(patch_feature, torch.unsqueeze(att[:, idx, :, :], dim=1))
        ...
        features_32x = self.gltrans_32x(global_features[4], global_features[4])  # B * H*W * C

        return att, global_features[1:], features_32x, local_features
    ...
    def forward(self, input):
        ...
        # Now put the features into the decoder. TODO: add ape?
        features_32x = self.gltrans_32x(local_features, local_features)
        return [global_features[0], global_features[1], global_features[2], global_features[3], features_32x]

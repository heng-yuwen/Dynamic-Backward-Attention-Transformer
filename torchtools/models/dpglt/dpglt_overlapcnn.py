...
class PatchEmbedConv(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_conv = nn.Conv2d(in_chans, mid_chans, kernel_size=self.patch_size, padding=1, stride=1)
        self.pool = nn.AvgPool2d(kernel_size=4 if scale_rate == 8 else 2)
        ...
        if scale_rate > 1:
            self.enlarge_conv = PatchMerging(mid_chans, scale_rate=scale_rate)
    ...
    def forward(self, x):
        ...
        if self.scale_rate > 1:
            x_enlarged = self.enlarge_conv(x, Wh, Ww)
        else:
            x_enlarged = None
        ...
        if self.norm_enlarge is not None:
            Wh, Ww = Wh // self.scale_rate, Ww // self.scale_rate
...
class DPGLTransformer(nn.Module, EncoderMixin):
    ...
    def __init__(self,
        ...
        self.patch_convs4x = PatchEmbedConv(3, in_chans, mid_chans=mid_chans[0], out_chans=self._out_channels[4],
                                            norm_layer=norm_layer if self.patch_norm else None, scale_rate=32//4)
        self.patch_convs8x = PatchEmbedConv(3, mid_chans[0], mid_chans=mid_chans[1], out_chans=self._out_channels[4],
                                            norm_layer=norm_layer if self.patch_norm else None, scale_rate=32//8)
        self.patch_convs16x = PatchEmbedConv(3, mid_chans[1], mid_chans=mid_chans[2], out_chans=self._out_channels[4],
                                             norm_layer=norm_layer if self.patch_norm else None, scale_rate=32//16)
        self.patch_convs32x = PatchEmbedConv(3, mid_chans[2], mid_chans=mid_chans[3], out_chans=self._out_channels[4],

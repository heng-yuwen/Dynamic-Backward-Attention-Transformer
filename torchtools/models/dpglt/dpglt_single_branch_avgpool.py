...
class EnlargeChannel(nn.Module):
    def __init__(self, in_chans, out_chans, scale_rate, norm_layer=nn.LayerNorm):
        ...
        if scale_rate > 1:
            self.avg_pool = nn.AvgPool2d(kernel_size=scale_rate)
    ...
    def forward(self, x):
        ...
        x = self.avg_pool(x)
        x = x.flatten(2).transpose(1, 2)
        x = self.enlarge_conv(x, H//self.scale_rate, W//self.scale_rate)
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, dim, norm_layer=nn.LayerNorm, scale_rate=2):
        super().__init__()
        self.dim = dim
        self.reduction = nn.Linear(dim, scale_rate * dim, bias=False)
        self.norm = norm_layer(dim)
        self.scale_rate = scale_rate

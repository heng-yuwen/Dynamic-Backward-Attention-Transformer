...
import torchtools.models.paccrf.pac as pac
...
class PacCRF(nn.Module):
    ...
    """
    def __init__(self, channels, num_steps=5, final_output='log_Q', perturbed_init=True, native_impl=False,
    ...
    def reset_parameters(self, pairwise_idx=None):
        ...
        for i in idxs:
            ...
            if self._use_pairwise_weights[i]:
    ...
    def forward(self, unary, edge_feat, edge_kernel=None, logQ=None, truth=None):
        ...
        for step in range(self.num_steps):
            ...
            if truth is not None:
                # set known ground truth pixels to zero first
                Q[:,:, truth.squeeze() != -1] = 0
                truth_labels = th.unique(truth)
                for label in truth_labels:
                    if label != -1:
                        Q[:, label, truth.squeeze() == label] = 1
        ...
        if self.final_output == 'softmax':
            ...
            if truth is not None:
                # set known ground truth pixels to zero first
                out[:,:, truth.squeeze() != -1] = 0
                truth_labels = th.unique(truth)
                for label in truth_labels:
                    if label != -1:
                        out[:, label, truth.squeeze() == label] = 1
...
class PacCRFLoose(nn.Module):
    ...
    def forward(self, unary, edge_feat, edge_kernel=None, truth=None):
        ...
        for step in self.steps:
            logQ = step(unary, None, edge_kernel, logQ, truth=truth)

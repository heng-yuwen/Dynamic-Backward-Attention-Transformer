...
class CKA:
    ...
    def _log_layer(self,
        ...
        if model == "model1":
            torch.save(out, "./model1_feature/{}.pt".format(name))
            self.model1_features.append(name)

        elif model == "model2":
            self.model2_features.append(name)
            torch.save(out, "./model2_feature/{}.pt".format(name))
    ...
    def _insert_hooks(self):
        ...
        for name, layer in self.model1.named_modules():
            if self.model1_layers is not None:
                if layer in self.model1_layers:
        ...
        for name, layer in self.model2.named_modules():
            if self.model2_layers is not None:
                if layer in self.model2_layers:
    ...
    def compare(self,
        ...
        for (x1, *_), (x2, *_) in pit(zip(dataloader1, dataloader2), total=num_batches, desc="Comparing Features", unit="ticks", color="red"):
            os.mkdir("./model1_feature")
            os.mkdir("./model2_feature")
            ...
            for i, name1 in pit(enumerate(self.model1_features), total=N, desc="Model1 Features", unit="ticks", color="green"):
                feat1 = torch.load("./model1_feature/{}.pt".format(name1))
                X = feat1.view(batch_size, -1)

                K = X @ X.t()
                K.fill_diagonal_(0.0)
                self.hsic_matrix[i, :, 0] += self._HSIC(K, K) / num_batches
                for j, name2 in pit(enumerate(self.model2_features), total=M, desc="Model2 Features", unit="ticks", color="blue"):

                    feat2 = torch.load("./model2_feature/{}.pt".format(name2))
                    Y = feat2.view(batch_size, -1)
            ...
            shutil.rmtree("./model1_feature")
            shutil.rmtree("./model2_feature")

        self.cka = self.hsic_matrix[:, :, 1] / (self.hsic_matrix[:, :, 0].sqrt() *
    ...
    def plot_results(self,
                     ...
                     title: str = None):
        fig, ax = plt.subplots()
        im = ax.imshow(self.cka, origin='lower', cmap='magma')

...
from torchtools.datasets.minc_zhao import MINCPatch, MINCSegment
...
class PatchLoader(LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        train_set = MINCPatch(root_dir=self._data_root, set_type='train',
    ...
    def val_dataloader(self):
        ...
        val_set = MINCPatch(root_dir=self._data_root, set_type='test',
                       classes=self._classes, transform=val_trans)
        val_set, _ = random_split(val_set, [16000, 80747], generator=torch.Generator().manual_seed(42))
    ...
    def test_dataloader(self):
        ...
        test_set = MINCPatch(root_dir=self._data_root, set_type='test',
                        classes=self._classes, transform=test_trans)
        _, test_set = random_split(test_set, [16000, 80747], generator=torch.Generator().manual_seed(42))
...
class SceneLoader(LightningDataModule):
    def __init__(self, data_root, json_data):
        super().__init__()
        self._use_gpu = torch.cuda.is_available()
        self._data_root = data_root
    ...
    def train_dataloader(self):
        train_set = MINCSegment(root_dir=self._data_root, set_type='train',
    ...
    def test_dataloader(self):
        test_set = MINCSegment(root_dir=self._data_root, set_type='test',
                        classes=self._classes)
        print("Test set loaded, with {} samples".format(len(test_set)))
        # if torch.cuda.device_count() > 1:
        sampler = DistributedSampler(test_set, shuffle=False)

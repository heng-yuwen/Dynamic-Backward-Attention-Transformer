...
from torchtools.datasets.opsurface import OPSurface
from torchtools.models.swin.helper import images_transform, masks_transform
import numpy as np
...
def collate(batch):
    ...
    material_segments = segments_tensor[:, :, :, 0]
    object_segments = segments_tensor[:, :, :, 1]
    scene_labels = segments_tensor[:, :, :, 2]  # single label
    segments_tensor = [material_segments, object_segments, scene_labels]

    images_255 = [torch.from_numpy(np.array(image)).permute(2, 0, 1).unsqueeze(dim=0) for image in images]
...
def collate_test(batch):
    ...
    material_segments = [segment[:, :, :, 0] for segment in segments_tensor]
    object_segments = [segment[:, :, :, 1] for segment in segments_tensor]
    scene_labels = [segment[:, 0, 0, 2] for segment in segments_tensor]  # single label
    segments_tensor = [material_segments, object_segments, scene_labels]
...
class OPSLoader(LightningDataModule):
    def __init__(self, data_root, batch_size=1, classes=45):
        super().__init__()
    ...
    def train_dataloader(self):
        train_set = OPSurface(root_dir=self._data_root, set_type='train',
                            use_transform=True)
    ...
    def val_dataloader(self):
        val_set = OPSurface(root_dir=self._data_root, set_type='validate')
    ...
    def test_dataloader(self):
        test_set = OPSurface(root_dir=self._data_root, set_type='test')
